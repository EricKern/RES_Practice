...
// g++ --std=c++11 -o ucLoadSer ucLoadSer.cpp 
// call like this:
// ./ucLoadSer -f ../sw/obj/main_simple.bin -s 0x8000 -p /dev/ttyUSB0 -c 1
...
void usage(string p) {
    ...
    << "[-p <serial port, default: /dev/ttyUSB0>] " << endl \
...
int send(int fd, vector<uint8_t> cmd, int dbgLen = 0) {
    int i;
   for (i = 0; i < cmd.size(); i++){
        if (1 != write(fd,&cmd.data()[i],1)) break;
        if (0 == (i % 10)) {
            tcdrain(fd);
        }
        usleep(100);
...
int main(int argc, char **argv){
    ...
    // open tty
    serFd = open(serDev.c_str(), O_RDWR | O_NOCTTY );
